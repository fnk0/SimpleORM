...
import com.gabilheri.simpleorm.R;
...
public class QueryUtils {
        ...
        if (cursor.moveToFirst()) {
            do {
                try {
                    ...
                    tORM.setOrmID(cursor.getLong(cursor.getColumnIndex(OrmObject.ID_FIELD)));
                    T ormObj = tORM.build();
    ...
    public static <T> List<T> getAll(Class<T> obj, SQLiteDatabase db, Context context) {
        return getListFromQuery(obj, db, context, context.getString(R.string.select_all));
    }
